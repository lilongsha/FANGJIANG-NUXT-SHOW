<template>
    <div class="fixed">
        <div class=" fixed w-full h-full top-0 left-0 z-[70] flex flex-col items-center justify-center" style="background: rgba(0,0,0,0.7); overflow: hidden; position: fixed;" @click="$emit('isOpen')">
            <div class="bg-right-top bg-auto bg-clue-head bg-no-repeat rounded-md fixed z-[80] opacity-100 bg-white lg:p-9 lg: w-[433px] lg: h-[458px] sm:w-[95%] sm:h-[320px] sm:p-3 sm:m-3 sm:space-y-3 sm:mx-3" @click="re">
                <div class="">
                ...
                <ClueLeaveClue v-show="step === 4" class="absolute z-[60] w-full h-full"  :project-id="projectId" :clue-type="'3'" :demand="value.toString()" @isOpen="$emit('isOpen')" />
                ...
                <div class=" lg:mt-11">
                    <button class=" sm:w-full sm:text-[15px] lg: w-[357px] lg:h-[55px] rounded-md text-white text-2xl bg-fjBlue-100" @click="nextStep">下一步</button>
                </div>
            </div>
        </div>
    </div>
