<template>
  <div class="">
    ...
    <div class="w-full sm:mt-1 lg:mt-6">
      <div v-for="item in context" :key="item.id" class="sm:mb-2 sm:px-2 lg:p-4 lg:mb-12 hover:bg-gray-100">
        <a :href="`/house/${item.id}.html`" target="_blank" class="w-full sm:h-36 lg:h-[250px] flex flex-row">
          <!-- 左边图片 -->
          <div class="sm:h-24 lg:h-[180px] lg:py-[35px]  sm:w-1/3 lg:w-[360px] static">
          ...
          <div class="h-full sm:w-2/3 lg:w-[920px] sm:pl-2 lg:pl-6">
            ...
            <div class="sm:mt-1 sm:text-[12px] lg:text-[16px]">
              ...
              <!-- 类型 -->
              <!-- <span v-if="item.type === '1'" class="">住宅</span>
            ...
            <div class="flex flex-row w-full lg:mt-7 lg:h-[168px] text-[#999999]">
              <div class="sm:w-full lg:w-3/5 h-full sm:text-[12px] lg:text-[18px]">
                ...
                <div class="lg:hidden w-full text-[14px] lg:text-right">
                  <!-- 咨询热线 -->
                  <a :href="`tel:${phoneNum},${item.number}%23`">
                    <span class="text-gray-400">咨询热线：</span>
                    <span class="text-sm font-bold text-fjRed-100">{{ phoneNum }}转{{ item.number }}</span>
                  </a>
                </div>
